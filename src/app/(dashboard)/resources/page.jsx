...
export default function ResourcesPage() {
  ...
  const fetchResources = async ({ silent = false } = {}) => {
    try {
      ...
      setResources(list.map(r => ({ ...r, isPremium: Boolean(r.isPremium) })))
    } catch {
  ...
  return (
    <>
      <DashboardHeader
        title='Resources'
        description='Manage learning content shown in the Discovery section of the app.'
      />
        ...
        <div className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
          <div className='relative w-full sm:max-w-sm'>
            ...
            <Input
              ...
              placeholder='Search title, type, pillar, category…'
              className='pl-8'
            />
          </div>
          ...
          <CardContent className='p-0'>
            {loading ? (
              ...
              <p className='py-12 text-center text-sm text-muted-foreground'>
                {resources.length === 0 ? 'No resources yet.' : 'No matches for your search.'}
              </p>
            ) : (
              <>
      ...
      <ResourceFormDialog
        ...
        resource={selected}
        onSaved={() => fetchResources({ silent: true })}
      />

      <ResourceViewDialog open={viewOpen} onOpenChange={setViewOpen} resource={selected} />

      <ConfirmDialog
        open={Boolean(deleteId)}
        onOpenChange={open => !open && setDeleteId(null)}
        title='Delete resource?'
        description='This will permanently remove the resource from the library.'
        confirmText='Delete'
        destructive
        onConfirm={async () => {
