...
function SidebarFooter() {
  ...
  return (
    <div className='space-y-2 p-3'>
      <div className='rounded-2xl border border-sidebar-border bg-sidebar-accent/80 p-3 shadow-sm'>
        <div className='flex items-center gap-3'>
          <Avatar className='size-9 ring-2 ring-sidebar-border ring-offset-2 ring-offset-sidebar'>
            <AvatarFallback className='bg-primary font-semibold text-primary-foreground'>{initials}</AvatarFallback>
          </Avatar>
          <div className='min-w-0 flex-1'>
            <p className='truncate text-sm font-semibold text-sidebar-foreground'>{user?.name || 'Staff'}</p>
            <p className='truncate text-xs text-muted-foreground'>
              {user?.isSuperAdmin
                ? 'Super-admin'
                : user?.role === 'editor'
                  ? 'Instructor'
                  : user?.adminLabel || user?.email}
            </p>
          </div>
        </div>
        <Button
          ...
          className='mt-3 w-full justify-start gap-2 border-dashed border-sidebar-border bg-transparent text-muted-foreground hover:border-destructive/40 hover:bg-destructive/5 hover:text-destructive'
        >
...
function SidebarBody({ onNavigate, collapsed = false }) {
  ...
  return (
    <div className='relative flex h-full flex-col overflow-hidden bg-sidebar'>
      <div
        className={cn(
          'relative flex h-14 shrink-0 items-center border-b border-sidebar-border',
        ...
        {collapsed ? (
          ...
          >
            <img
              src='/logo.png'
              alt='Trader 365'
              className='size-9 shrink-0 rounded-xl object-contain ring-1 ring-sidebar-border'
            />
          </button>
        ) : (
          <>
...
export function AppSidebar() {
  ...
  return (
    <aside
      className={cn(
        'sticky top-0 hidden h-screen shrink-0 overflow-hidden border-r border-sidebar-border bg-sidebar text-sidebar-foreground transition-[width] duration-200 ease-in-out md:flex md:flex-col',
        open
          ? 'w-72 shadow-[4px_0_24px_-12px_rgba(0,0,0,0.12)] dark:shadow-[4px_0_24px_-12px_rgba(0,0,0,0.45)]'
          : 'w-[4.5rem]'
      )}
